...
(function(root, factory) {
	...
	return angular.module('gridster', [])
		...
		.constant('gridsterConfig', {
			...
			ratio: 0, // minumum row height of an item
			jumpX: 1, // minumum row height of an item
			jumpY: 1, // minumum row height of an item
						...
						if(item.ratio>0){
							if(item.ratio>=1){
								if (['n', 's'].indexOf(handleClass) === -1) {
									...
									sizeX = Math.ceil(sizeX/item.ratio)*(item.ratio);
									sizeY=sizeX/item.ratio;
								}
								else if (['e', 'w'].indexOf(handleClass) === -1) {
									sizeY = gridster.pixelsToColumns(elmH, true);
									sizeX=sizeY*item.ratio;
								}
							}
							else{
								if (['n', 's'].indexOf(handleClass) === -1) {
									...
									sizeY = gridster.pixelsToColumns(elmH, true);
									sizeY = Math.ceil(sizeY*item.ratio)/(item.ratio);
									sizeX=sizeY*item.ratio;
							...
							sizeX = Math.ceil(sizeX/item.jumpX)*(item.jumpX);
							sizeY = Math.ceil(sizeY/item.jumpY)*(item.jumpY);
			...
			function($parse, GridsterDraggable, GridsterResizable, gridsterDebounce) {
				return {
					...
					link: function(scope, $el, attrs, controllers) {
						...
						var aspects = ['minSizeX', 'maxSizeX', 'minSizeY', 'ratio','draggable','resizable','jumpX','jumpY', 'maxSizeY', 'sizeX', 'sizeY', 'row', 'col'],
							$getters = {};
						...
						var updateResizable = function() {
							//console.log(options.resizable);
							resizable.toggle(options.resizable && !gridster.isMobile && gridster.resizable && gridster.resizable.enabled);
						};
						updateResizable();

						var updateDraggable = function() {
							draggable.toggle(options.draggable && !gridster.isMobile && gridster.draggable && gridster.draggable.enabled);
						};
						updateDraggable();
